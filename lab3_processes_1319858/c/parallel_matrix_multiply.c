...
int main(int argc, char **argv) {
    ...
    if (!equal_matrices(seq, par, rows_a, cols_b)) {
    ...
    if (write_matrix_to_file(argv[3], par, rows_a, cols_b) != 0) {
        free_matrix(a, rows_a);
        free_matrix(b, rows_b);
        free_matrix(seq, rows_a);
        free_matrix(par, rows_a);
    ...
    double seq_seconds = elapsed_seconds(&start_seq, &end_seq);
    double par_seconds = elapsed_seconds(&start_par, &end_par);
